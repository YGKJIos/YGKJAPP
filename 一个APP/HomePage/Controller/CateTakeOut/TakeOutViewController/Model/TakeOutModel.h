//
//  TakeOutModel.h
//  一个APP
//
//  Created by 梁立彬 on 16/7/15.
//  Copyright © 2016年 llb. All rights reserved.
//

#import <Foundation/Foundation.h>

@interface TakeOutModel : NSObject
@property (nonatomic, strong)NSString *shangjiaId;
@property (nonatomic, strong)NSString *shangjiaName;
@property (nonatomic, strong)NSString *shangjiaPingfen;
@property (nonatomic, strong)NSString *shangjiaWeizhi;
@property (nonatomic, strong)NSString *shangjiaJuli;
@property (nonatomic, strong)NSString *shangjiaTongzhi;
@property (nonatomic, strong)NSString *waimaiQisongjia;
@property (nonatomic, strong)NSString *waimaiPeisongfei;
@property (nonatomic, strong)NSString *shangjiaTouxiang;

@end
